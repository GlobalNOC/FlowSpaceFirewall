...
public class Proxy {
	...
	public void setAdminStatus(Boolean status){
		...
		if(status){
			log.debug("Slice: "+ this.mySlicer.getSliceName() +" is re-enabled");
		}else{

			log.info("Disabling Slice:"+this.mySlicer.getSliceName() );
			if(this.connected()){
				this.removeFlows();
				this.disconnect();
			}
	...
	private void processFlowMod(OFMessage msg, FloodlightContext cntx){
		...
		if(this.mySlicer.getTagManagement()){
			...
			if(flows.size() ==0){
				log.info("Slice: " + this.mySlicer.getSliceName() + ":" + this.getSlicer().getSwitchName() + " denied flow: " + ((OFFlowMod)msg).toString());
		...
		while(it.hasNext()){
			...
			case OFFlowMod.OFPFC_ADD:
				...
				if( this.mySlicer.isGreaterThanMaxFlows(this.flowCount + 1) ) {
					log.info("Switch: "+this.getSlicer().getSwitchName()+" Slice: "+this.mySlicer.getSliceName()+" Flow count is already at threshold. Skipping flow mod");
	...
	public void toSwitch(OFMessage msg, FloodlightContext cntx){
		...
		if(!this.mySlicer.isOkToProcessMessage()){
			log.info("Switch: "+this.getSlicer().getSwitchName()+"Slice:"+this.mySlicer.getSliceName()+"Rate limit exceeded");
			OFError error = new OFError(OFError.OFErrorType.OFPET_BAD_REQUEST);
			error.setErrorCode(OFBadRequestCode.OFPBRC_EPERM);
			this.sendError((OFMessage)msg,error);
			return;
	...
	public void toController(OFMessage msg, FloodlightContext cntx){
		...
		case PACKET_IN:
			...
			if(this.packetInRate.okToProcess()){
				...
				log.info("Packet in Rate for Slice: " +
